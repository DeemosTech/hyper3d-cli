...
import {
  ...
  compareInput,
  validateInput,
} from '../packages/cli/dist/schema.js';
...
test('ignores descriptions and ordering but preserves property names', () => {
  const other = {
    ...
    properties: { mode: { enum: ['b', 'a'], type: 'string' } },
  };
  assert.deepEqual(compareInput(base, other), []);
  assert.ok(
    canonical({ properties: { description: { type: 'string' } } }).properties
      .description,
  );
  assert.notDeepEqual(
    canonical({ const: { title: 'a' } }),
    canonical({ const: { title: 'b' } }),
  );
});
test('new optional properties are compatible; required additions and enum narrowing are not', () => {
  assert.deepEqual(
    compareInput(base, {
      ...base,
      properties: { ...base.properties, extra: { type: 'number' } },
    }),
    [],
  );
  assert.equal(
    compareInput(base, { ...base, required: ['mode', 'extra'] })[0].severity,
    'breaking',
  );
  assert.equal(
    compareInput(base, {
...
test('complex changes and output changes are unknown; missing tool is unavailable', () => {
  assert.equal(
    compareInput(base, { ...base, anyOf: [{ required: ['mode'] }] })[0]
      .severity,
    'unknown',
  );
  assert.equal(
    compareTool(
      { inputSchema: base },
      { inputSchema: base, outputSchema: { type: 'object' } },
    )[0].path,
    '$output',
  );
