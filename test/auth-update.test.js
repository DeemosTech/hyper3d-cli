...
test('credentials are endpoint-bound, private, and removable', async () => {
  ...
  try {
    ...
    if (process.platform !== 'win32') assert.equal((await stat(credentialPath(endpoint))).mode & 0o777, 0o600);
    await assert.rejects(async () => provider.redirectToAuthorization(new URL('https://example.com/authorize')), /auth login/);
...
test('npm checks use semver and validate channels', async () => {
  assert.equal((await checkUpdate({name: '@hyper3d/cli', version: '0.9.0'}, 'latest', async () => '"0.10.0"')).available, true);
  await assert.rejects(checkUpdate({name: 'x', version: '1.0.0'}, 'bad', async () => '"2.0.0"'));
});
test('updates only the global installation using an exact version', async () => {
  ...
  const calls = [];
  const run = async args => { calls.push(args); return args[0] === 'root' ? root : args[0] === 'view' ? '"0.2.0"' : ''; };
  try {
    const result = await update({name: '@hyper3d/cli', version: '0.1.0'}, directory, 'latest', run);
    assert.equal(result.updated, true);
    assert.deepEqual(calls.at(-1), ['install', '--global', '@hyper3d/cli@0.2.0', '--no-fund', '--no-audit']);
    await assert.rejects(update({name: '@hyper3d/cli', version: '0.1.0'}, root, 'latest', run), /Not a global/);
    await assert.rejects(update({private: true}, directory, 'latest', run), /private/);
  } finally { await rm(root, {recursive: true, force: true}); }
});
test('minimum policy blocks old versions; network failure never implies forced update', async () => {
  assert.equal(evaluatePolicy({schemaVersion: 1, minimumVersion: '0.2.0'}, '0.1.0').required, true);
  assert.throws(() => evaluatePolicy({schemaVersion: 1, minimumVersion: 'bad'}, '0.1.0'));
  await assert.rejects(enforcePolicy('https://example.com/policy.json', '0.1.0', async () => ({ok: true, json: async () => ({schemaVersion: 1, minimumVersion: '0.2.0'})})), /CLIENT_UPGRADE_REQUIRED/);
  await enforcePolicy('https://example.com/policy.json', '0.1.0', async () => { throw new Error('offline'); });
});
