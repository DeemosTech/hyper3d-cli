...
import type { Change, Payload } from './types.js';
import type { Tool } from '@modelcontextprotocol/sdk/types.js';

const annotations = new Set(['description', 'title', '$comment', 'examples']);
export function canonical(value: any, keyword = '', literal = false): any {
...
export function compareInput(before: any, after: any, path = '$'): Change[] {
  ...
  const add = (severity: string, reason: string, at = path) =>
    changes.push({ severity, path: at, reason });
  if (!equal(before.type, after.type)) add('unknown', 'type changed');
  for (const name of after.required ?? [])
    if (!(before.required ?? []).includes(name))
      add('breaking', 'new required property', `${path}.${name}`);
  if (
    after.enum &&
    (!before.enum ||
      before.enum.some(
        (v: unknown) => !after.enum.some((w: unknown) => equal(v, w)),
      ))
  )
    add('breaking', 'accepted enum values narrowed');
  for (const [name, schema] of Object.entries(before.properties ?? {})) {
    if (!Object.hasOwn(after.properties ?? {}, name))
      add('unknown', 'property removed', `${path}.${name}`);
    else
      changes.push(
        ...compareInput(schema, after.properties[name], `${path}.${name}`),
      );
  }
  for (const name of Object.keys(after.properties ?? {})) {
    if (
      !Object.hasOwn(before.properties ?? {}, name) &&
      before.additionalProperties !== false
    )
      add(
        'unknown',
        'new property constrains previously unrestricted input',
        `${path}.${name}`,
      );
  }
...
export function compareTool(before: Tool | undefined, after: Tool | undefined) {
  if (!after)
    return [
      {
        severity: 'breaking',
        path: '$',
        reason: 'tool unavailable (removed or not authorized)',
      },
    ];
  if (!before) return [];
